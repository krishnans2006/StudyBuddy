...
class Info(commands.Cog, name="help", description="Get info about the bot and how to use it"):
    ...
    async def cmd_help(self, client, context, command):
        ...
        embed = create_embed("Help", description=f"Help with the `{command}` command", fields=[
    ...
    async def show_help(self, context, name: Optional[str]):
        ...
        if get(self.client.commands, name=name):
            ...
            await context.send(embed=create_embed("Help", description=f"The command `{name}` does not exist!"))

    @commands.command(name="info", aliases=["about", "faq", "faqs"], brief="Get information about the bot", description="This will allow you to get information about the bot")
    async def info(self, context):
        await context.send(embed=create_embed("Info", description=self.client.info))
    
    @commands.command(name="how-to", aliases=["getstarted", "howto"], brief="Learn how to start using the bot", description="This will give you detailed instructions on how to get started with the bot")
    async def howto(self, context):
        await context.send(embed=create_embed("How-To", description=self.client.howto))
